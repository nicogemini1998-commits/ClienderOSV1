...
export default function CompanyCard({ lead, onClick, onStatusChange, onPhoneRevealed, index = 0 }) {
  ...
  return (
    <m.article
      ...
      className="bg-gradient-to-br from-surface-card to-surface border border-surface-border/60 rounded-xl cursor-pointer group overflow-hidden shadow-[0_4px_12px_-2px_rgba(0,0,0,0.2)] hover:border-accent/40 transition-colors duration-200 p-0"
      initial={{ opacity: 0, y: 18 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: index * 0.05, duration: 0.35, ease: 'easeOut' }}
      whileHover={{ y: -3, boxShadow: '0 12px 28px -4px rgba(0,0,0,0.35)' }}
      whileTap={{ scale: 0.985 }}
    >
      {/* Accent top stripe */}
      <div className={`h-1 w-full ${opp.dot} opacity-90`} />

      <div className="p-4">
        {/* Header: name + score */}
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="min-w-0 flex-1">
            <h3 className="text-white font-semibold text-base leading-tight truncate group-hover:text-accent transition-colors duration-200">
              {company.name}
            </h3>
          ...
          <div className="flex-shrink-0 text-right">
            <div className={`text-2xl font-black leading-none tabular-nums ${scoreColor(company.digital_score)}`}
                 style={{ textShadow: company.digital_score >= 65 ? '0 0 20px rgb(52 211 153 / 0.4)' : company.digital_score >= 35 ? '0 0 20px rgb(251 191 36 / 0.4)' : '0 0 20px rgb(248 113 113 / 0.4)' }}>
        ...
        <div className="bg-surface-raised border border-surface-border rounded-lg p-3 mb-3 group-hover:border-accent/15 transition-colors duration-200">
