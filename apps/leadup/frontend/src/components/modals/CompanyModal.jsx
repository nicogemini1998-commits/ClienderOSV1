...
import RemindersList from '../sections/RemindersList'
...
function ReportContent({ content }) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('# ')) {
      ...
      blocks.push(<h3 key={i} className="text-sm font-semibold text-accent uppercase tracking-wider mt-4 mb-2">{line.slice(4)}</h3>)
      ...
      blocks.push(
        <ol key={`ol-${i}`} className="space-y-1.5 my-2">
          {items.map((item, j) => (
            <li key={j} className="flex gap-2 text-sm text-slate-300 leading-relaxed">
              <span className="text-accent font-semibold flex-shrink-0 w-4">{j + 1}.</span>
              <span>{renderInline(item)}</span>
            </li>
          ))}
        </ol>
              ...
              <thead>
                <tr className="bg-surface-raised">
                  {parseRow(rows[0]).map((cell, j) => (
                    <th key={j} className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-3 py-2 border-b border-surface-border">
                      {renderInline(cell)}
                    </th>
                  ))}
                </tr>
      ...
      <div className="relative bg-gradient-to-br from-surface-raised to-surface border border-surface-border rounded-2xl w-full max-w-2xl max-h-[92vh] flex flex-col animate-slide-up overflow-hidden shadow-[0_20px_48px_-12px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-start justify-between p-8 pb-5 border-b border-surface-border/60 bg-gradient-to-r from-surface-raised/40 to-transparent">
          <div className="min-w-0 flex-1 flex items-start gap-4">
            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${AVATAR_COLORS[company.name.charCodeAt(0) % AVATAR_COLORS.length]} flex items-center justify-center flex-shrink-0 font-semibold text-white text-lg shadow-[0_4px_12px_-2px_rgba(0,0,0,0.3)]`}>
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-surface-border/50 bg-gradient-to-r from-surface-card/60 to-surface-card/20">
          <span className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold border shadow-[0_2px_6px_-1px_rgba(0,0,0,0.2)] ${oppStyle}`}>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-7">
                ...
                {editingContact ? (
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label htmlFor="contact-name" className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Nombre</label>
                        <input
                          id="contact-name"
                          type="text"
                      ...
                      <div>
                        <label htmlFor="contact-title" className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Cargo</label>
                        <input
                          id="contact-title"
                          type="text"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label htmlFor="contact-phone" className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Teléfono</label>
                        <input
                          id="contact-phone"
                          type="tel"
                      ...
                      <div>
                        <label htmlFor="contact-email" className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Email</label>
                        <input
                          id="contact-email"
                          type="email"
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      {isRevealed && mobile ? (
                        <a
                          href={`tel:${mobile}`}
                          onClick={(e) => e.stopPropagation()}
                          className="flex items-center gap-2 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 rounded-xl px-4 py-2.5 font-mono font-semibold text-sm transition-colors flex-shrink-0"
                        >
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
                        ...
                        <a
                          href={`tel:${mobile}`}
                          onClick={(e) => e.stopPropagation()}
                          className="flex items-center gap-2 bg-accent/10 hover:bg-accent/20 border border-accent/30 text-accent rounded-xl px-4 py-2.5 font-mono font-semibold text-sm transition-colors flex-shrink-0"
                        >
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
                            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 9a16 16 0 0 0 7.09 7.09l.41-.41a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 17.92z" />
                          </svg>
          ...
          <section>
            <SectionLabel>Oportunidades Cliender</SectionLabel>
            ...
            <section>
              ...
              <div className="mb-3 flex items-center gap-2.5 px-3 py-2.5 bg-accent/8 border border-accent/20 rounded-xl">
                ...
                <p className="text-xs text-slate-300">
                  Script personalizado para <span className="font-semibold text-accent">{comercialName}</span> · {company.name}
                </p>
              </div>
              <ol className="space-y-2">
                {company.opening_lines.map((line, i) => {
                  const personalizedLine = line
                    .replace(/soy\s+\w+\s+de\s+HBD/gi, `soy ${comercialName} de Cliender`)
                    .replace(/soy\s+Nicolás/gi, `soy ${comercialName}`)
                    .replace(/de\s+HBD/gi, 'de Cliender')
                  return (
