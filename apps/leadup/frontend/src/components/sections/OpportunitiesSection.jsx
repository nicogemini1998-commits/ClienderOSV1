    ...
    <section aria-labelledby="opp-heading">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {cached && enrichment && (
            <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-400/10 text-amber-400/80 border border-amber-400/20">
              Cache
            </span>
          )}
        </div>
      ...
      {!loading && !error && enrichment && (
        <div className="bg-surface-card border border-surface-border rounded-xl overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-surface-border/50">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${levelStyle.cls}`}>
              {levelStyle.label}
            </span>
                ...
                <ul className="space-y-2">
                  {enrichment.hooks.map((hook, i) => (
                    <li key={i} className="flex items-start gap-2 group">
                      <span className="text-accent flex-shrink-0 mt-0.5 text-xs font-semibold">›</span>
